...
//  TODO: Your dataset filename. Expected format is CSV
  private static final String FILENAME = "../project2/data/bank-dataset.csv";

  //  TODO: How many examples you have
  private static int num_examples = 5000;

  //  TODO: How many attributes you have. This is (number_of_columns - 1)
  private static int num_attributes = 16;
...
//  TODO: Set this to false if you retained the best SA params from a previous run
   ...
   * @param args ignored
   */
  // args: PokerTest <shouldFindParams: boolean> <backpropIterations: int> <rhcIterations: int> <saIterations: int> <gaIterations: int>
  public static void main(String[] args) {
    boolean shouldFindParams = args.length >= 1 && Boolean.parseBoolean(args[0]);
    int backpropIterations = args.length >= 2 ? Integer.parseInt(args[1]) : 200;
    int rhcIterations = args.length >= 3 ? Integer.parseInt(args[2]) : 2000;
    int saIterations = args.length >= 4 ? Integer.parseInt(args[3]) : 2000;
    int gaIterations = args.length >= 5 ? Integer.parseInt(args[4]) : 20;

    System.out.println(shouldFindParams ? "Will find params" : "Will NOT search for params");
    System.out.printf("backprop iterations: %d%n", backpropIterations);
    System.out.printf("rhc iterations: %d%n", rhcIterations);
    System.out.printf("sa iterations: %d%n", saIterations);
    System.out.printf("ga iterations: %d%n", gaIterations);
...
//    Determine best NN params, which happen to just be the number of hidden layer nodes.
    if (shouldFindNNParams && shouldFindParams) {
...
//      This is how Weka does it: # of attributes + # of classes / 2
    ...
    /* Backprop */
    // trainIterations = 200;
    trainIterations = backpropIterations;
...
//      Find the average error for this network configuration
        ...
        error /= validationErrors.length;
        System.out.printf("temp: %s, cooling: %s, error: %s%n", df.format(temp), df.format(cooling), df.format(error));
        errors[x][y] = error;
...
//      Find the average error for this configuration
          ...
          error /= validationErrors.length;
          System.out.printf("population: %d, mate: %s, mutate: %s, error: %s%n", population, df.format(mate), df.format(mutate), df.format(error));
          errors[x][y][z] = error;
