...
def initialize_instances():
    ...
    INPUT_FILE = os.path.join("..", "..", "project1", "data", "eyes.csv")
...
def train(oa, network, oaName, instances, measure, test_instances):
    ...
    for iteration in xrange(TRAINING_ITERATIONS):
        ...
        print '{},{}'.format("%0.03f" % error, "%0.03f" % test_error)
...
def main():
    ...
    for i, name in enumerate(oa_names):
        ...
        n = len(instances)
        train_instances = instances[:int(n/2)]
        test_instances = instances[int(n/2):]
